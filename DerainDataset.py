...
def prepare_data_Rain100H(data_path, patch_size, stride):
    # train
...
def prepare_data_Rain100L(data_path, patch_size, stride):
    # train
    print('process training data')
    input_path = os.path.join(data_path)
    target_path = os.path.join(data_path)
...
def prepare_data_Rain800(data_path, patch_size, stride):
    ...
    target_path = os.path.join(data_path, 'rainy')
    ...
    for i in range(700):
        ...
        if os.path.exists(os.path.join(target_path,target_file)):
            ...
            target_img = target[0:h,0:w//2,0:c]
            input_img = target[0:h,w//2:w,0:c]
            #print("target_img  = %s,input_img = %s"%(target_img.shape,input_img.shape))
